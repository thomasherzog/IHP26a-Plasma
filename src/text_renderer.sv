module text_renderer (
  ...
  function automatic [7:0] get_font_row(input logic [7:0] char, input logic [2:0] row);
    case (char)
...
function automatic get_text_pixel(input [9:0] x, input [9:0] y);
    logic [7:0] char_code;
    logic [9:0] font_x;
    logic [9:0] font_y;
    ...
    begin
      ...
      // 22 chars * 16px = 352px wide. To re-center on 640px, start at x=144, end at x=496.
  ...
  wire up_pix     = get_text_pixel(hpos,         vpos - 10'd1);
  wire down_pix   = get_text_pixel(hpos,         vpos + 10'd1);
  wire left_pix   = get_text_pixel(hpos - 10'd1, vpos);
  wire right_pix  = get_text_pixel(hpos + 10'd1, vpos);
  wire ul_pix     = get_text_pixel(hpos - 10'd1, vpos - 10'd1);
  wire ur_pix     = get_text_pixel(hpos + 10'd1, vpos - 10'd1);
  wire dl_pix     = get_text_pixel(hpos - 10'd1, vpos + 10'd1);
  wire dr_pix     = get_text_pixel(hpos + 10'd1, vpos + 10'd1);

  // A pixel is a border if the center is empty, but any neighbor has text
  assign is_border = (up_pix | down_pix | left_pix | right_pix | ul_pix | ur_pix | dl_pix | dr_pix) & ~is_inner;
